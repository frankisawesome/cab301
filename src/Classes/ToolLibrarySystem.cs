...
namespace cab301
{
    class ToolLibrarySystem : iToolLibrarySystem
    {
        ...
        //helper method to flatten all tools in a dictionary into a list
        private void addToolsToArray (Dictionary<string, ToolCollection> dictionary, List<Tool> list)
        {
            foreach (KeyValuePair<string, ToolCollection> toolCollection in dictionary)
            {
                foreach (Tool tool in toolCollection.Value.toArray())
                {
                    list.Add(tool);
                }
            }
        ...
        {
            //Flattens all collections into one array
            List<Tool> allTools = new List<Tool>();
            addToolsToArray(gardeningTools, allTools);
            //use quicksort
        }

        // Partition for quicksort, adopted from pseudocode
        private int partition(Tool[] tools, int l, int r)
        {
            /*
            ...
            while(true){
                ...
                do {
                    i++;
                } while (tools[i].NoBorrowings > tools.NoBorrowings);
                do {
                    j--;
                } while (tools[j].NoBorrowings < p.NoBorrowings);
         ...
         // sort (Quicksort pt2)
        private void sort(Tool[] tools, int l, int r){
            /*
            ...
            if (l < r){
                int s = partition(tools, l, r); // split position
                sort(tools, l, s);
                sort(tools,s+1, r);
            }
        }
